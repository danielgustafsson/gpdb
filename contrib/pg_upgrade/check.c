				  ...
				  char **sequence_script_file_name)
{
	...
	/* Check to see if external tables are used as partitions */
 ...
 *
 *	External tables used in partition hierarchies via ALTER TABLE EXCHANGE
 *	cannot be restored during the upgrade since EXCHANGE isn't allowed in
 *	utility mode. Issue a warning to the operator in case any such partitions
 *	exist in the old cluster since they will have to be manually re-added via
 *	the specific EXTTAB_DUMP_FILE dump file. This dumpfile is generated by
 *	pg_dump as part of the binary upgrade dump mode.
 *
 *	Additionally, since we need to dispatch the Oids for the created dummy
 *	partition (which will be exchanged with the external table) to the QEs
 *	during the upgrade. Save the information on which tables we need to deal
 *	with later on and issue a warning to the user that a dumpfile must be
 *	manually moved to the QEs during the upgrade. Once the new cluster has
 *	been restored in the QD we need to backfill the data saved here with the
 *	Oids that the relations were allocated. The reason we can't do all that
 *	in one go, like how we handle the arraytypes, is that we at this point
 *	don't know which partitions will be exchanged when we are in the new
 *	cluster.
 */
static void
check_external_partition(migratorContext *ctx)
{
	...
	for (dbnum = 0; dbnum < old_cluster->dbarr.ndbs; dbnum++)
	{
		...
		if (ntups > 0)
		{
			...
			/*
			 * There might already be dispatch objects associated with the
			 * current database, reallocate the array to fit the new objects if
			 * so. We must allocate twice the number of tuples since each
			 * relation created generates a relation Oid and a type Oid which
			 * we need. There is an additional arraytype Oid generated as well
			 * but since we deal with arraytypes specifically in
			 * old_GPDB4_find_array_types() we can skip them here.
			 *
			 * GPDB_84MERGE_FIXME: Once we need to upgrade from GPDB 5.0 we
			 * need to pull in the arraytype here as well since the wholesale
			 * grabbing of arraytypes only applies to upgrades from 4.3. Rather
			 * than second-guess now what we need for arraytypes in that
			 * upgrade however, let's cross that bridge when we get there.
			 */
			if (active_db->dispatch_arr.ndispatch > 0)
			{
				int size = (active_db->dispatch_arr.ndispatch + (ntups * 2)) * sizeof(DispatchInfo);
				active_db->dispatch_arr.dispatches = pg_realloc(ctx, &active_db->dispatch_arr.dispatches, size);
			}
			else
				active_db->dispatch_arr.dispatches = pg_malloc(ctx, sizeof(DispatchInfo) * (ntups * 2));

			/*
			 * Until we can find out the final Oid which was assigned, set to
			 * InvalidOid for both relation and type to signal that we as yet
			 * don't know.
			 */
			dispno = active_db->dispatch_arr.ndispatch;

			for (rowno = 0; rowno < ntups; rowno++)
			{
				DispatchInfo *relation = &active_db->dispatch_arr.dispatches[dispno++];
				DispatchInfo *type = &active_db->dispatch_arr.dispatches[dispno++];

				relation->type = DISPATCH_RELATION;
				relation->oid = InvalidOid;
				strlcpy(relation->name, PQgetvalue(res, rowno, PQfnumber(res, "partname")), sizeof(relation->name));
				relation->namespace_oid = atooid(PQgetvalue(res, rowno, PQfnumber(res, "relnamespace")));

				type->type = DISPATCH_TYPE;
				type->oid = InvalidOid;
				strlcpy(type->name, PQgetvalue(res, rowno, PQfnumber(res, "partname")), sizeof(type->name));
				type->namespace_oid = atooid(PQgetvalue(res, rowno, PQfnumber(res, "relnamespace")));
			}

			active_db->dispatch_arr.ndispatch += (ntups * 2);
		}
	...
	if (found)
	{
		pg_log(ctx, PG_REPORT, "\n\n"
			   ...
			   "| tables as partitions.  These partitions need to be re-added\n"
			   "| to the partition hierarchy after the upgrade.  A list of SQL\n"
			   "| commands to re-add these partitions is in the file:\n"
			   "| \t%s\n\n", EXTTAB_DUMP_FILE);

		pg_log(ctx, PG_REPORT,
			   "| In upgrading the partitioned tables with external tables as\n"
			   "| partitions, additional objects have been created.  In order\n"
