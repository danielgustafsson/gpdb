		...
		case RM_BTREE_ID:
			switch (info)
			{
				...
				case XLOG_BTREE_SPLIT_L_ROOT:
				{
					...
					char *datapos = XLogRecGetData(xlrec);
					...
					if (!(xl_info & XLR_BKP_BLOCK_1))
					{
						IndexTuple item = (IndexTuple) datapos;
						ChangeTracking_AddRelationChangeInfo(relationChangeInfoArray,
															 relationChangeInfoArrayCount,
															 relationChangeInfoMaxSize,
															 &(xlrec->node),
															 BlockIdGetBlockNumber(&item->t_tid.ip_blkid),
															 (ItemPointer) &item->t_tid,
															 0);
					}
